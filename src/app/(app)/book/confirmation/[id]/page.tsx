...
export default async function ConfirmationPage({ params }: { params: { id: string } }) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center p-6">
      <div className="w-20 h-20 bg-green-100 rounded-full flex items-center justify-center text-4xl mb-4 animate-bounce">✅</div>
      <h1 className="text-xl font-bold text-cc-dark mb-1">¡Reserva confirmada!</h1>
      <p className="text-sm text-gray-500 mb-6">Tu court está reservado y el PIN de acceso está activo</p>
      ...
      {booking.pin_code && (
        <div className="w-full bg-cc-blue rounded-2xl p-6 text-white text-center mb-4">
          ...
          <p className="text-xs opacity-70 mt-2">Válido {booking.start_time?.slice(0,5)} – {booking.end_time?.slice(0,5)}</p>
      ...
      <div className="w-full bg-cc-blue/10 rounded-xl p-3 text-xs text-cc-blue mb-6">
        🔑 Introduce el PIN en el teclado de la puerta para acceder. Se desactiva automáticamente al finalizar tu sesión.
      </div>

      <Link href="/home" className="btn-primary w-full text-center">Volver al inicio</Link>
