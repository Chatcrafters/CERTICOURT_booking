    ...
    <div className="pt-14">
      ...
      <div className="bg-cc-dark text-white px-5 pb-5">
        ...
        <div className="grid grid-cols-2 gap-3">
          {[
            ...
            { val: expiring.length, lbl: 'Contratos expiran', warn: true },
          ].map((k,i) => (
            <div key={i} className="bg-white/10 rounded-xl p-3 text-center">
              <div className="text-xl font-bold font-mono">{k.val}</div>
              <div className="text-xs text-gray-400 mt-0.5 flex items-center justify-center gap-1">{k.warn && <IconWarning size={12} />}{k.lbl}</div>
            </div>
          ))}
        </div>
      </div>
        ...
        <div className="space-y-2">
          {courts?.map(c => (
            <div key={c.id} className="bg-white rounded-2xl p-3 border border-gray-100 flex items-center gap-3">
              <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-cc-blue to-cc-teal flex items-center justify-center text-white"><IconCourt size={20} /></div>
      ...
      {expiring.length > 0 && (
        <div className="mx-4 mb-4 bg-amber-50 border border-amber-200 rounded-2xl p-4">
          <h3 className="text-sm font-bold text-amber-800 mb-2 flex items-center gap-1"><IconWarning size={16} /> Contratos proximos a vencer</h3>
          ...
          <Link href="/operator/sponsoring" className="block text-center text-xs font-semibold text-amber-700 mt-2">Ver todos &rarr;</Link>
        </div>
      )}
    </div>
  )
