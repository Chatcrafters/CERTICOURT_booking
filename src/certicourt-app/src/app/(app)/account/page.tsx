...
export default async function AccountPage() {
  ...
  const menuItems = [
    { href: '/agenda',           label: 'Mis reservas' },
    { href: '/agenda',           label: 'Mis llaves / PIN activos' },
    { href: '/account/bonos',    label: 'Bonos y abonos' },
    { href: '/account/payments', label: 'Pagos y facturas' },
    { href: '/discover',         label: 'Mis centros favoritos' },
    { href: '/operator',         label: 'Panel de Operador', highlight: true },
  ]
    ...
    <div>
      ...
      <div className="mx-4 -mt-5 bg-cc-blue rounded-2xl p-5 text-white shadow-lg">
        ...
        <div className="flex gap-2 mt-3">
          <button className="flex-1 py-2 rounded-xl border-2 border-white/30 bg-white/10 text-sm font-semibold">Recargar</button>
          <button className="flex-1 py-2 rounded-xl border-2 border-white/30 bg-white/10 text-sm font-semibold">Historial</button>
        </div>
      </div>
      ...
      <div className="mt-5 divide-y divide-gray-100">
        {menuItems.map(item => (
          <Link key={item.href + item.label} href={item.href}
            ...
            <span className={`flex-1 text-sm font-medium ${item.highlight ? 'text-cc-blue font-semibold' : 'text-cc-dark'}`}>
              {item.label}
            </span>
        ...
        <form action="/api/auth/signout" method="POST">
          <button type="submit" className="flex items-center gap-4 px-5 py-4 w-full text-left">
