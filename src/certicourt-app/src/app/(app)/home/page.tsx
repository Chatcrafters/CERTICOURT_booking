...
export default async function HomePage() {
  ...
  const quickActions = [
    { href: '/book',     label: 'Reservar',   sub: 'Busca un court ahora' },
    { href: '/agenda',   label: 'Mis llaves', sub: 'Codigos activos' },
    { href: '/account',  label: 'Bonos',      sub: 'Paquetes y abonos' },
    { href: '/discover', label: 'Centros',    sub: 'Descubrir' },
  ]
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 gap-3 p-4 -mt-4">
        {quickActions.map(a => (
          <Link key={a.href} href={a.href}
            ...
            <div className="w-8 h-8 rounded-lg bg-cc-blue-light mb-2 flex items-center justify-center">
      ...
      <div className="px-4 mb-4">
        ...
        <div className="space-y-2">
          {courts?.map(c => (
            <Link key={c.id} href={`/book?court=${c.id}`}
              className="bg-white rounded-2xl p-4 border border-gray-100 flex items-center gap-3 active:scale-95 transition-transform block">
              <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-cc-blue to-cc-teal flex-shrink-0"></div>
