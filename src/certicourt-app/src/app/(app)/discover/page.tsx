    ...
    <div>
      <div className="bg-white px-5 pt-14 pb-4 border-b border-gray-100">
        ...
        <div className="flex items-center gap-3 bg-gray-50 rounded-xl px-4 py-3 border border-gray-200">
          <svg width="16" height="16" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24" className="text-gray-400 flex-shrink-0"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.35-4.35"/></svg>
          <input className="flex-1 bg-transparent text-sm outline-none text-cc-dark placeholder-gray-400" placeholder="Buscar centros..." />
        </div>
        <div className="flex gap-2 mt-3 overflow-x-auto scrollbar-hide pb-1">
          {['Todos', 'Pickleball', 'Padel', 'Tenis', 'Cerca de mi'].map((f, i) => (
      ...
      <div className="divide-y divide-gray-100">
        {centers?.map(center => (
          <Link key={center.id} href={`/book?center=${center.slug}`}
            ...
            <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-cc-blue to-cc-teal flex items-center justify-center text-white font-bold text-lg flex-shrink-0">
              {center.name.replace('CERTICOURT ', '').slice(0, 2).toUpperCase()}
            </div>
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-2 mt-1">
                {center.wpc_certified && (
                  <span className="text-xs bg-cc-blue-light text-cc-blue font-semibold px-2 py-0.5 rounded-full font-mono">WPC</span>
                )}
